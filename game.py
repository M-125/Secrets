...
class Main:
    def __init__(self, fps, window_width, window_height):
        ...
        self.player_state = HookedDict(direction = "down", coords = (self.window_width // 2, self.window_height // 2))
    ...
    def update_player(self, item_id, old_value, new_value): # When the self.player_state dictionary is updated, this method is called. Allows management of the players state in a single central area.
        if item_id == "coords":
            ...
            self.player.play_sheet(new_value)
        else:
            pass
    ...
    def __postinit__(self):
        ...
        self.player_animation_sheet = AnimationSheet(
            ...
            player_idle_up=self.idle_up_sheet,
            player_idle_down=self.idle_down_sheet,
            player_idle_left=self.idle_left_sheet,
            player_idle_right=self.idle_right_sheet,
            player_walking_up=self.walking_up_sheet,
            player_walking_down=self.walking_down_sheet,
            player_walking_left=self.walking_left_sheet,
            player_walking_right=self.walking_right_sheet
        )
